/**
 *  @file   MergeFunc.cpp
 *  @brief  This is a copy o LLVM's mergefunc implementation, that works on set
 *of functions
...
MergeFunc::runOnSet(std::set<Function *> &SF) {
    bool Changed = false;
    ...
    for (Function *Func : SF) {
        if (isEligibleForMerging(*Func)) {
            HashedFuncs.push_back(
                {FunctionComparator::functionHash(*Func), Func});
        }
    }
    ...
    std::map<Function *, Function *> delToNew;
    do {
        ...
        for (WeakTrackingVH &I : Worklist) {
            ...
            if (!F->isDeclaration() && !F->hasAvailableExternallyLinkage()) {
                auto K = insert(
                    F); // {if was merged, {function* deleted, function* new}}
                Changed |= K.first;
                delToNew[K.second.first] = K.second.second;
            }
        }
...
void MergeFunc::replaceDirectCallers(Function *Old, Function *New) {
    ...
    for (Use &U : llvm::make_early_inc_range(Old->uses())) {
        ...
        if (CB && CB->isCallee(&U)) {
            ...
            // Function comparison ensures that the attributes are the same up
            // to type congruences in byval(), in which case we need to keep the
            // byval type of the call-site, not the callee function.
...
static Value *createCast(IRBuilder<> &Builder, Value *V, Type *DestTy) {
    ...
    if (SrcTy->isStructTy()) {
        ...
        for (unsigned int I = 0, E = SrcTy->getStructNumElements(); I < E;
             ++I) {
            Value *Element =
...
void MergeFunc::eraseInstsUnrelatedToPDI(
    ...
    LLVM_DEBUG(
        dbgs() << " } // Done erasing instructions unrelated to parameter "
...
void MergeFunc::filterInstsUnrelatedToPDI(
    ...
    for (BasicBlock::iterator BI = GEntryBlock->begin(),
                              BIE = GEntryBlock->end();
         BI != BIE; ++BI) {
        if (auto *DVI = dyn_cast<DbgValueInst>(&*BI)) {
            ...
            if (DILocVar->isParameter()) {
                ...
                AllocaInst *AI =
                    dyn_cast_or_null<AllocaInst>(DDI->getAddress());
                if (AI) {
...
static bool canCreateAliasFor(Function *F) {
    if (/*!MergeFunctionsAliases ||*/ !F->hasGlobalUnnamedAddr()) return false;
...
void MergeFunc::writeAlias(Function *F, Function *G) {
    ...
    auto *GA =
        GlobalAlias::create(G->getValueType(), PtrType->getAddressSpace(),
...
void MergeFunc::mergeTwoFunctions(Function *F, Function *G) {
    if (F->isInterposable()) {
        ...
        // Both writeThunkOrAlias() calls below must succeed, either because we
        // can create aliases for G and NewF, or because a thunk for F is
        // profitable. F here has the same signature as NewF below, so that's
        // what we check.
        if (!canCreateThunkFor(F) &&
        ...
        // We collect alignment before writeThunkOrAlias that overwrites NewF
        // and G's content.
        ...
        if (NewFAlign || GAlign)
            F->setAlignment(
                std::max(NewFAlign.valueOrOne(), GAlign.valueOrOne()));
        else
            F->setAlignment(std::nullopt);
        ...
        // For better debugability, under MergeFunctionsPDI, we do not modify
        // G's call sites to point to F even when within the same translation
        // unit.
        if (!G->isInterposable() /*&& !MergeFunctionsPDI*/) {
            // Functions referred to by llvm.used/llvm.compiler.used are
            // special: there are uses of the symbol name that are not visible
            // to LLVM, usually from inline asm.
            if (G->hasGlobalUnnamedAddr() && !Used.contains(G)) {
                // G might have been a key in our GlobalNumberState, and it's
                // illegal to replace a key in ValueMap<GlobalValue *> with a
                // non-global.
        ...
        // If G was internal then we may have replaced all uses of G with F. If
        // so, stop here and delete G. There's no need for a thunk. (See note on
        // MergeFunctionsPDI above).
...
void MergeFunc::replaceFunctionInTree(const FunctionNode &FN, Function *G) {
...
static bool isFuncOrderCorrect(const Function *F, const Function *G) {
    ...
    if (F->hasLocalLinkage() != G->hasLocalLinkage()) {
        // External before local, because we definitely have to keep the
        // external function, but may be able to drop the local one.
...
std::pair<bool, std::pair<Function *, Function *>>
MergeFunc::insert(Function *NewFunction) {
    ...
    if (Result.second) {
        ...
        LLVM_DEBUG(dbgs() << "Inserting as unique: " << NewFunction->getName()
                          << '\n');
        return {false, std::make_pair(nullptr, nullptr)};
    }
    ...
    return {true, std::make_pair(DeleteF, OldF.getFunc())};
}
...
void MergeFunc::remove(Function *F) {
    ...
    if (I != FNodesInTree.end()) {
        ...
        // I->second has been invalidated, remove it from the FNodesInTree map
...
void MergeFunc::removeUsers(Value *V) {
    for (User *U : V->users())
        if (auto *I = dyn_cast<Instruction>(U)) remove(I->getFunction());
}
