...
int main() {
  ...
  // Runtime-dependent solver state
  double solver_clause_activity = 0.0;
  int solver_clauses_size = 10; // Example size, can be set at runtime

  // Allocate at runtime
  Clause **solver_clauses =
      (Clause **)malloc(sizeof(Clause *) * solver_clauses_size);
  Clause **solver_clauses_data1 =
      (Clause **)malloc(sizeof(Clause *) * solver_clauses_size);
  char *solver_clauses_data2 =
      (char *)malloc(sizeof(char) * solver_clauses_size);

  // Initialize with mock data for demonstration
  for (int idx = 0; idx < solver_clauses_size; ++idx) {
    solver_clauses[idx] = (Clause *)malloc(sizeof(Clause));
    solver_clauses[idx]->size = 24;
    solver_clauses[idx]->activity = (float)idx;
    solver_clauses[idx]->data[0] = idx * 2;
    solver_clauses[idx]->data[1] = idx * 2 + 1;
    solver_clauses_data1[idx] = solver_clauses[idx];
    solver_clauses_data2[idx] = 1;
  }
  solver_clause_activity = 5.0;
...
block93:
  ...
  if (threshold > (double)temp_float)
    goto block98; // Original fcmp
  else
    goto block100;
