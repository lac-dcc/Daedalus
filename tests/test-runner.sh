...
command -v clang >/dev/null 2>&1 || {
  echo >&2 "clang is required but it's not installed. Aborting..."
  exit 1
}
command -v llvm-objcopy >/dev/null 2>&1 || {
  echo >&2 "llvm-objcopy is required but it's not installed. Aborting..."
  exit 1
}
command -v opt >/dev/null 2>&1 || {
  echo >&2 "opt is required but it's not installed. Aborting..."
  exit 1
}
command -v FileCheck >/dev/null 2>&1 || {
  echo >&2 "FileCheck is required but it's not installed. Aborting..."
  exit 1
}
