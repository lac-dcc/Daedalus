; ModuleID = 'encode.e.bc.ll'
source_filename = "ld-temp.o"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.g72x_state = type { i64, i16, i16, i16, i16, [2 x i16], [6 x i16], [2 x i16], [6 x i16], [2 x i16], i8 }

@pack_output.out_buffer = internal unnamed_addr global i32 0, align 4
@pack_output.out_bits = internal unnamed_addr global i32 0, align 4
@stdout = external local_unnamed_addr global ptr, align 8
@stderr = external local_unnamed_addr global ptr, align 8
@.str = private unnamed_addr constant [31 x i8] c"CCITT ADPCM Encoder -- usage:\0A\00", align 1
@.str.1 = private unnamed_addr constant [46 x i8] c"\09encode [-3|4|5] [-a|u|l] < infile > outfile\0A\00", align 1
@.str.2 = private unnamed_addr constant [8 x i8] c"where:\0A\00", align 1
@.str.3 = private unnamed_addr constant [40 x i8] c"\09-3\09Generate G.723 24kbps (3-bit) data\0A\00", align 1
@.str.4 = private unnamed_addr constant [50 x i8] c"\09-4\09Generate G.721 32kbps (4-bit) data [default]\0A\00", align 1
@.str.5 = private unnamed_addr constant [40 x i8] c"\09-5\09Generate G.723 40kbps (5-bit) data\0A\00", align 1
@.str.6 = private unnamed_addr constant [36 x i8] c"\09-a\09Process 8-bit A-law input data\0A\00", align 1
@.str.7 = private unnamed_addr constant [46 x i8] c"\09-u\09Process 8-bit u-law input data [default]\0A\00", align 1
@.str.8 = private unnamed_addr constant [42 x i8] c"\09-l\09Process 16-bit linear PCM input data\0A\00", align 1
@stdin = external local_unnamed_addr global ptr, align 8
@qtab_721 = internal global [7 x i16] [i16 -124, i16 80, i16 178, i16 246, i16 300, i16 349, i16 400], align 2
@_dqlntab = internal unnamed_addr constant [16 x i16] [i16 -2048, i16 4, i16 135, i16 213, i16 273, i16 323, i16 373, i16 425, i16 425, i16 373, i16 323, i16 273, i16 213, i16 135, i16 4, i16 -2048], align 16
@_witab = internal unnamed_addr constant [16 x i16] [i16 -12, i16 18, i16 41, i16 64, i16 112, i16 198, i16 355, i16 1122, i16 1122, i16 355, i16 198, i16 112, i16 64, i16 41, i16 18, i16 -12], align 16
@_fitab = internal unnamed_addr constant [16 x i16] [i16 0, i16 0, i16 0, i16 512, i16 512, i16 512, i16 1536, i16 3584, i16 3584, i16 1536, i16 512, i16 512, i16 512, i16 0, i16 0, i16 0], align 16
@qtab_723_24 = internal global [3 x i16] [i16 8, i16 218, i16 331], align 2
@_dqlntab.5 = internal unnamed_addr constant [8 x i16] [i16 -2048, i16 135, i16 273, i16 373, i16 373, i16 273, i16 135, i16 -2048], align 16
@_witab.6 = internal unnamed_addr constant [8 x i16] [i16 -128, i16 960, i16 4384, i16 18624, i16 18624, i16 4384, i16 960, i16 -128], align 16
@_fitab.7 = internal unnamed_addr constant [8 x i16] [i16 0, i16 512, i16 1024, i16 3584, i16 3584, i16 1024, i16 512, i16 0], align 16
@qtab_723_40 = internal global [15 x i16] [i16 -122, i16 -16, i16 68, i16 139, i16 198, i16 250, i16 298, i16 339, i16 378, i16 413, i16 445, i16 475, i16 502, i16 528, i16 553], align 16
@_dqlntab.10 = internal unnamed_addr constant [32 x i16] [i16 -2048, i16 -66, i16 28, i16 104, i16 169, i16 224, i16 274, i16 318, i16 358, i16 395, i16 429, i16 459, i16 488, i16 514, i16 539, i16 566, i16 566, i16 539, i16 514, i16 488, i16 459, i16 429, i16 395, i16 358, i16 318, i16 274, i16 224, i16 169, i16 104, i16 28, i16 -66, i16 -2048], align 16
@_witab.11 = internal unnamed_addr constant [32 x i16] [i16 448, i16 448, i16 768, i16 1248, i16 1280, i16 1312, i16 1856, i16 3200, i16 4512, i16 5728, i16 7008, i16 8960, i16 11456, i16 14080, i16 16928, i16 22272, i16 22272, i16 16928, i16 14080, i16 11456, i16 8960, i16 7008, i16 5728, i16 4512, i16 3200, i16 1856, i16 1312, i16 1280, i16 1248, i16 768, i16 448, i16 448], align 16
@_fitab.12 = internal unnamed_addr constant [32 x i16] [i16 0, i16 0, i16 0, i16 0, i16 0, i16 512, i16 512, i16 512, i16 512, i16 512, i16 1024, i16 1536, i16 2048, i16 2560, i16 3072, i16 3072, i16 3072, i16 3072, i16 2560, i16 2048, i16 1536, i16 1024, i16 512, i16 512, i16 512, i16 512, i16 512, i16 0, i16 0, i16 0, i16 0, i16 0], align 16
@power2 = internal unnamed_addr constant [15 x i16] [i16 1, i16 2, i16 4, i16 8, i16 16, i16 32, i16 64, i16 128, i16 256, i16 512, i16 1024, i16 2048, i16 4096, i16 8192, i16 16384], align 16

; Function Attrs: nounwind optsize uwtable
define dso_local i32 @main(i32 noundef %0, ptr nocapture noundef readonly %1) #0 {
  %3 = alloca %struct.g72x_state, align 8
  %4 = alloca i8, align 1
  %5 = alloca i16, align 2
  call void @llvm.lifetime.start.p0(i64 56, ptr nonnull %3) #16
  call void @llvm.lifetime.start.p0(i64 1, ptr nonnull %4) #16
  call void @llvm.lifetime.start.p0(i64 2, ptr nonnull %5) #16
  store i64 34816, ptr %3, align 8, !tbaa !7
  %6 = getelementptr inbounds %struct.g72x_state, ptr %3, i64 0, i32 1
  store i16 544, ptr %6, align 8, !tbaa !13
  %7 = getelementptr inbounds %struct.g72x_state, ptr %3, i64 0, i32 2
  %8 = getelementptr inbounds i8, ptr %3, i64 32
  store i32 0, ptr %8, align 8
  call void @llvm.memset.p0.i64(ptr noundef nonnull align 2 dereferenceable(10) %7, i8 0, i64 10, i1 false)
  %9 = getelementptr inbounds i8, ptr %3, i64 20
  call void @llvm.memset.p0.i64(ptr noundef nonnull align 4 dereferenceable(12) %9, i8 0, i64 12, i1 false), !tbaa !14
  %10 = getelementptr inbounds %struct.g72x_state, ptr %3, i64 0, i32 8, i64 0
  store <8 x i16> <i16 32, i16 32, i16 32, i16 32, i16 32, i16 32, i16 32, i16 32>, ptr %10, align 4, !tbaa !14
  %11 = getelementptr inbounds %struct.g72x_state, ptr %3, i64 0, i32 10
  store i8 0, ptr %11, align 4, !tbaa !15
  %12 = icmp sgt i32 %0, 1
  br i1 %12, label %13, label %61

13:                                               ; preds = %53, %2
  %14 = phi i32 [ %58, %53 ], [ 4, %2 ]
  %15 = phi ptr [ %57, %53 ], [ @g721_encoder, %2 ]
  %16 = phi ptr [ %56, %53 ], [ %4, %2 ]
  %17 = phi i32 [ %55, %53 ], [ 1, %2 ]
  %18 = phi i32 [ %54, %53 ], [ 1, %2 ]
  %19 = phi i32 [ %59, %53 ], [ %0, %2 ]
  %20 = phi ptr [ %21, %53 ], [ %1, %2 ]
  %21 = getelementptr inbounds ptr, ptr %20, i64 1
  %22 = load ptr, ptr %21, align 8, !tbaa !16
  %23 = load i8, ptr %22, align 1, !tbaa !18
  %24 = icmp eq i8 %23, 45
  br i1 %24, label %25, label %61

25:                                               ; preds = %13
  %26 = getelementptr inbounds i8, ptr %22, i64 1
  %27 = load i8, ptr %26, align 1, !tbaa !18
  %28 = sext i8 %27 to i32
  switch i32 %28, label %34 [
    i32 51, label %53
    i32 52, label %29
    i32 53, label %30
    i32 117, label %31
    i32 97, label %32
    i32 108, label %33
  ]

29:                                               ; preds = %25
  br label %53

30:                                               ; preds = %25
  br label %53

31:                                               ; preds = %25
  br label %53

32:                                               ; preds = %25
  br label %53

33:                                               ; preds = %25
  br label %53

34:                                               ; preds = %25
  %35 = load ptr, ptr @stderr, align 8, !tbaa !16
  %36 = tail call i64 @fwrite(ptr nonnull @.str, i64 30, i64 1, ptr %35) #17
  %37 = load ptr, ptr @stderr, align 8, !tbaa !16
  %38 = tail call i64 @fwrite(ptr nonnull @.str.1, i64 45, i64 1, ptr %37) #17
  %39 = load ptr, ptr @stderr, align 8, !tbaa !16
  %40 = tail call i64 @fwrite(ptr nonnull @.str.2, i64 7, i64 1, ptr %39) #17
  %41 = load ptr, ptr @stderr, align 8, !tbaa !16
  %42 = tail call i64 @fwrite(ptr nonnull @.str.3, i64 39, i64 1, ptr %41) #17
  %43 = load ptr, ptr @stderr, align 8, !tbaa !16
  %44 = tail call i64 @fwrite(ptr nonnull @.str.4, i64 49, i64 1, ptr %43) #17
  %45 = load ptr, ptr @stderr, align 8, !tbaa !16
  %46 = tail call i64 @fwrite(ptr nonnull @.str.5, i64 39, i64 1, ptr %45) #17
  %47 = load ptr, ptr @stderr, align 8, !tbaa !16
  %48 = tail call i64 @fwrite(ptr nonnull @.str.6, i64 35, i64 1, ptr %47) #17
  %49 = load ptr, ptr @stderr, align 8, !tbaa !16
  %50 = tail call i64 @fwrite(ptr nonnull @.str.7, i64 45, i64 1, ptr %49) #17
  %51 = load ptr, ptr @stderr, align 8, !tbaa !16
  %52 = tail call i64 @fwrite(ptr nonnull @.str.8, i64 41, i64 1, ptr %51) #17
  tail call void @exit(i32 noundef 1) #18
  unreachable

53:                                               ; preds = %33, %32, %31, %30, %29, %25
  %54 = phi i32 [ 3, %33 ], [ 2, %32 ], [ 1, %31 ], [ %18, %30 ], [ %18, %29 ], [ %18, %25 ]
  %55 = phi i32 [ 2, %33 ], [ 1, %32 ], [ 1, %31 ], [ %17, %30 ], [ %17, %29 ], [ %17, %25 ]
  %56 = phi ptr [ %5, %33 ], [ %4, %32 ], [ %4, %31 ], [ %16, %30 ], [ %16, %29 ], [ %16, %25 ]
  %57 = phi ptr [ %15, %33 ], [ %15, %32 ], [ %15, %31 ], [ @g723_40_encoder, %30 ], [ @g721_encoder, %29 ], [ @g723_24_encoder, %25 ]
  %58 = phi i32 [ %14, %33 ], [ %14, %32 ], [ %14, %31 ], [ 5, %30 ], [ 4, %29 ], [ 3, %25 ]
  %59 = add nsw i32 %19, -1
  %60 = icmp sgt i32 %19, 2
  br i1 %60, label %13, label %61, !llvm.loop !19

61:                                               ; preds = %53, %13, %2
  %62 = phi i32 [ 1, %2 ], [ %18, %13 ], [ %54, %53 ]
  %63 = phi i32 [ 1, %2 ], [ %17, %13 ], [ %55, %53 ]
  %64 = phi ptr [ %4, %2 ], [ %16, %13 ], [ %56, %53 ]
  %65 = phi ptr [ @g721_encoder, %2 ], [ %15, %13 ], [ %57, %53 ]
  %66 = phi i32 [ 4, %2 ], [ %14, %13 ], [ %58, %53 ]
  %67 = zext i32 %63 to i64
  %68 = icmp eq i32 %63, 2
  br label %69

69:                                               ; preds = %76, %61
  %70 = phi i32 [ undef, %61 ], [ %84, %76 ]
  %71 = load ptr, ptr @stdin, align 8, !tbaa !16
  %72 = call i64 @fread(ptr noundef %64, i64 noundef %67, i64 noundef 1, ptr noundef %71) #19
  %73 = icmp eq i64 %72, 1
  br i1 %73, label %76, label %74

74:                                               ; preds = %69
  %.lcssa = phi i32 [ %70, %69 ]
  %75 = icmp eq i32 %.lcssa, 0
  br i1 %75, label %88, label %85

76:                                               ; preds = %69
  %77 = load i16, ptr %5, align 2
  %78 = sext i16 %77 to i32
  %79 = load i8, ptr %4, align 1
  %80 = zext i8 %79 to i32
  %81 = select i1 %68, i32 %78, i32 %80
  %82 = call i32 (i32, i32, ptr, ...) %65(i32 noundef %81, i32 noundef %62, ptr noundef nonnull %3) #20, !callees !21
  %83 = and i32 %82, 255
  %84 = call fastcc i32 @pack_output(i32 noundef %83, i32 noundef %66) #19, !range !22
  br label %69, !llvm.loop !23

85:                                               ; preds = %85, %74
  %86 = call fastcc i32 @pack_output(i32 noundef 0, i32 noundef %66) #19, !range !22
  %87 = icmp eq i32 %86, 0
  br i1 %87, label %88, label %85, !llvm.loop !24

88:                                               ; preds = %85, %74
  %89 = load ptr, ptr @stdout, align 8, !tbaa !16
  %90 = call i32 @fclose(ptr noundef %89) #19
  call void @llvm.lifetime.end.p0(i64 2, ptr nonnull %5) #16
  call void @llvm.lifetime.end.p0(i64 1, ptr nonnull %4) #16
  call void @llvm.lifetime.end.p0(i64 56, ptr nonnull %3) #16
  ret i32 0
}

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.start.p0(i64 immarg, ptr nocapture) #1

; Function Attrs: nocallback nofree nounwind willreturn memory(argmem: write)
declare void @llvm.memset.p0.i64(ptr nocapture writeonly, i8, i64, i1 immarg) #2

; Function Attrs: nounwind optsize memory(read, argmem: readwrite, inaccessiblemem: none) uwtable
define internal i32 @g721_encoder(i32 noundef %0, i32 noundef %1, ptr nocapture noundef %2) unnamed_addr #3 {
  switch i32 %1, label %72 [
    i32 2, label %4
    i32 1, label %6
    i32 3, label %8
  ]

4:                                                ; preds = %3
  %5 = tail call i32 (i32, ...) @alaw2linear(i32 noundef %0) #20
  br label %8

6:                                                ; preds = %3
  %7 = tail call i32 (i32, ...) @ulaw2linear(i32 noundef %0) #20
  br label %8

8:                                                ; preds = %6, %4, %3
  %9 = phi i32 [ %7, %6 ], [ %5, %4 ], [ %0, %3 ]
  %10 = lshr i32 %9, 2
  %11 = tail call fastcc i32 @predictor_zero(ptr noundef %2) #20
  %12 = shl i32 %11, 16
  %13 = ashr exact i32 %12, 16
  %14 = ashr i32 %12, 17
  %15 = tail call fastcc i32 @predictor_pole(ptr noundef %2) #20
  %16 = add nsw i32 %13, %15
  %17 = shl i32 %16, 15
  %18 = ashr i32 %17, 16
  %19 = sub nsw i32 %10, %18
  %20 = tail call fastcc i32 @step_size(ptr noundef %2) #20
  %21 = shl i32 %19, 16
  %22 = ashr exact i32 %21, 16
  %23 = shl i32 %20, 16
  %24 = ashr exact i32 %23, 16
  %25 = tail call fastcc i32 @quantize(i32 noundef %22, i32 noundef %24, ptr noundef nonnull @qtab_721, i32 noundef 7) #20
  %26 = zext i32 %25 to i64
  %27 = shl i32 %25, 16
  %28 = ashr exact i32 %27, 16
  %29 = and i32 %25, 8
  %30 = shl i64 %26, 48
  %31 = ashr exact i64 %30, 48
  %32 = getelementptr inbounds [16 x i16], ptr @_dqlntab, i64 0, i64 %31
  %33 = load i16, ptr %32, align 2, !tbaa !14
  %34 = zext i16 %33 to i32
  %35 = lshr i32 %24, 2
  %36 = add nuw nsw i32 %35, %34
  %37 = and i32 %36, 32768
  %38 = icmp eq i32 %37, 0
  br i1 %38, label %42, label %39

39:                                               ; preds = %8
  %40 = icmp eq i32 %29, 0
  %41 = select i1 %40, i32 0, i32 -32768
  br label %53

42:                                               ; preds = %8
  %43 = lshr i32 %36, 7
  %44 = and i32 %43, 15
  %45 = shl i32 %36, 7
  %46 = and i32 %45, 16256
  %47 = or i32 %46, 16384
  %48 = sub nsw i32 14, %44
  %49 = lshr i32 %47, %48
  %50 = icmp eq i32 %29, 0
  %51 = or i32 %49, -32768
  %52 = select i1 %50, i32 %49, i32 %51
  br label %53

53:                                               ; preds = %42, %39
  %54 = phi i32 [ %41, %39 ], [ %52, %42 ]
  %55 = icmp slt i32 %54, 0
  %56 = and i32 %54, 16383
  %57 = sub nsw i32 0, %56
  %58 = select i1 %55, i32 %57, i32 %54
  %59 = add nsw i32 %58, %18
  %60 = shl i32 %59, 16
  %61 = ashr exact i32 %60, 16
  %62 = add nsw i32 %58, %14
  %63 = getelementptr inbounds [16 x i16], ptr @_witab, i64 0, i64 %31
  %64 = load i16, ptr %63, align 2, !tbaa !14
  %65 = sext i16 %64 to i32
  %66 = shl nsw i32 %65, 5
  %67 = getelementptr inbounds [16 x i16], ptr @_fitab, i64 0, i64 %31
  %68 = load i16, ptr %67, align 2, !tbaa !14
  %69 = sext i16 %68 to i32
  %70 = shl i32 %62, 16
  %71 = ashr exact i32 %70, 16
  tail call fastcc void @update(i32 noundef 4, i32 noundef %24, i32 noundef %66, i32 noundef %69, i32 noundef %54, i32 noundef %61, i32 noundef %71, ptr noundef %2) #20
  br label %72

72:                                               ; preds = %53, %3
  %73 = phi i32 [ %28, %53 ], [ -1, %3 ]
  ret i32 %73
}

; Function Attrs: nofree nounwind optsize
declare noundef i64 @fwrite(ptr nocapture noundef, i64 noundef, i64 noundef, ptr nocapture noundef) local_unnamed_addr #4

; Function Attrs: noreturn nounwind optsize
declare void @exit(i32 noundef) local_unnamed_addr #5

; Function Attrs: nounwind optsize memory(read, argmem: readwrite, inaccessiblemem: none) uwtable
define internal i32 @g723_40_encoder(i32 noundef %0, i32 noundef %1, ptr nocapture noundef %2) unnamed_addr #3 {
  switch i32 %1, label %70 [
    i32 2, label %4
    i32 1, label %6
    i32 3, label %8
  ]

4:                                                ; preds = %3
  %5 = tail call i32 (i32, ...) @alaw2linear(i32 noundef %0) #20
  br label %8

6:                                                ; preds = %3
  %7 = tail call i32 (i32, ...) @ulaw2linear(i32 noundef %0) #20
  br label %8

8:                                                ; preds = %6, %4, %3
  %9 = phi i32 [ %7, %6 ], [ %5, %4 ], [ %0, %3 ]
  %10 = lshr i32 %9, 2
  %11 = tail call fastcc i32 @predictor_zero(ptr noundef %2) #20
  %12 = shl i32 %11, 16
  %13 = ashr i32 %12, 17
  %14 = tail call fastcc i32 @predictor_pole(ptr noundef %2) #20
  %15 = add i32 %14, %11
  %16 = shl i32 %15, 16
  %17 = ashr i32 %16, 17
  %18 = sub nsw i32 %10, %17
  %19 = tail call fastcc i32 @step_size(ptr noundef %2) #20
  %20 = shl i32 %18, 16
  %21 = ashr exact i32 %20, 16
  %22 = shl i32 %19, 16
  %23 = ashr exact i32 %22, 16
  %24 = tail call fastcc i32 @quantize(i32 noundef %21, i32 noundef %23, ptr noundef nonnull @qtab_723_40, i32 noundef 15) #20
  %25 = zext i32 %24 to i64
  %26 = shl i32 %24, 16
  %27 = ashr exact i32 %26, 16
  %28 = and i32 %24, 16
  %29 = shl i64 %25, 48
  %30 = ashr exact i64 %29, 48
  %31 = getelementptr inbounds [32 x i16], ptr @_dqlntab.10, i64 0, i64 %30
  %32 = load i16, ptr %31, align 2, !tbaa !14
  %33 = zext i16 %32 to i32
  %34 = lshr i32 %23, 2
  %35 = add nuw nsw i32 %34, %33
  %36 = and i32 %35, 32768
  %37 = icmp eq i32 %36, 0
  br i1 %37, label %41, label %38

38:                                               ; preds = %8
  %39 = icmp eq i32 %28, 0
  %40 = select i1 %39, i32 0, i32 -32768
  br label %52

41:                                               ; preds = %8
  %42 = lshr i32 %35, 7
  %43 = and i32 %42, 15
  %44 = shl i32 %35, 7
  %45 = and i32 %44, 16256
  %46 = or i32 %45, 16384
  %47 = sub nsw i32 14, %43
  %48 = lshr i32 %46, %47
  %49 = icmp eq i32 %28, 0
  %50 = or i32 %48, -32768
  %51 = select i1 %49, i32 %48, i32 %50
  br label %52

52:                                               ; preds = %41, %38
  %53 = phi i32 [ %40, %38 ], [ %51, %41 ]
  %54 = icmp slt i32 %53, 0
  %55 = and i32 %53, 32767
  %56 = sub nsw i32 0, %55
  %57 = select i1 %54, i32 %56, i32 %53
  %58 = add nsw i32 %57, %17
  %59 = shl i32 %58, 16
  %60 = ashr exact i32 %59, 16
  %61 = add nsw i32 %57, %13
  %62 = getelementptr inbounds [32 x i16], ptr @_witab.11, i64 0, i64 %30
  %63 = load i16, ptr %62, align 2, !tbaa !14
  %64 = sext i16 %63 to i32
  %65 = getelementptr inbounds [32 x i16], ptr @_fitab.12, i64 0, i64 %30
  %66 = load i16, ptr %65, align 2, !tbaa !14
  %67 = sext i16 %66 to i32
  %68 = shl i32 %61, 16
  %69 = ashr exact i32 %68, 16
  tail call fastcc void @update(i32 noundef 5, i32 noundef %23, i32 noundef %64, i32 noundef %67, i32 noundef %53, i32 noundef %60, i32 noundef %69, ptr noundef %2) #20
  br label %70

70:                                               ; preds = %52, %3
  %71 = phi i32 [ %27, %52 ], [ -1, %3 ]
  ret i32 %71
}

; Function Attrs: nounwind optsize memory(read, argmem: readwrite, inaccessiblemem: none) uwtable
define internal i32 @g723_24_encoder(i32 noundef %0, i32 noundef %1, ptr nocapture noundef %2) unnamed_addr #3 {
  switch i32 %1, label %70 [
    i32 2, label %4
    i32 1, label %6
    i32 3, label %8
  ]

4:                                                ; preds = %3
  %5 = tail call i32 (i32, ...) @alaw2linear(i32 noundef %0) #20
  br label %8

6:                                                ; preds = %3
  %7 = tail call i32 (i32, ...) @ulaw2linear(i32 noundef %0) #20
  br label %8

8:                                                ; preds = %6, %4, %3
  %9 = phi i32 [ %7, %6 ], [ %5, %4 ], [ %0, %3 ]
  %10 = lshr i32 %9, 2
  %11 = tail call fastcc i32 @predictor_zero(ptr noundef %2) #20
  %12 = shl i32 %11, 16
  %13 = ashr i32 %12, 17
  %14 = tail call fastcc i32 @predictor_pole(ptr noundef %2) #20
  %15 = add i32 %14, %11
  %16 = shl i32 %15, 16
  %17 = ashr i32 %16, 17
  %18 = sub nsw i32 %10, %17
  %19 = tail call fastcc i32 @step_size(ptr noundef %2) #20
  %20 = shl i32 %18, 16
  %21 = ashr exact i32 %20, 16
  %22 = shl i32 %19, 16
  %23 = ashr exact i32 %22, 16
  %24 = tail call fastcc i32 @quantize(i32 noundef %21, i32 noundef %23, ptr noundef nonnull @qtab_723_24, i32 noundef 3) #20
  %25 = zext i32 %24 to i64
  %26 = shl i32 %24, 16
  %27 = ashr exact i32 %26, 16
  %28 = and i32 %24, 4
  %29 = shl i64 %25, 48
  %30 = ashr exact i64 %29, 48
  %31 = getelementptr inbounds [8 x i16], ptr @_dqlntab.5, i64 0, i64 %30
  %32 = load i16, ptr %31, align 2, !tbaa !14
  %33 = zext i16 %32 to i32
  %34 = lshr i32 %23, 2
  %35 = add nuw nsw i32 %34, %33
  %36 = and i32 %35, 32768
  %37 = icmp eq i32 %36, 0
  br i1 %37, label %41, label %38

38:                                               ; preds = %8
  %39 = icmp eq i32 %28, 0
  %40 = select i1 %39, i32 0, i32 -32768
  br label %52

41:                                               ; preds = %8
  %42 = lshr i32 %35, 7
  %43 = and i32 %42, 15
  %44 = shl i32 %35, 7
  %45 = and i32 %44, 16256
  %46 = or i32 %45, 16384
  %47 = sub nsw i32 14, %43
  %48 = lshr i32 %46, %47
  %49 = icmp eq i32 %28, 0
  %50 = or i32 %48, -32768
  %51 = select i1 %49, i32 %48, i32 %50
  br label %52

52:                                               ; preds = %41, %38
  %53 = phi i32 [ %40, %38 ], [ %51, %41 ]
  %54 = icmp slt i32 %53, 0
  %55 = and i32 %53, 16383
  %56 = sub nsw i32 0, %55
  %57 = select i1 %54, i32 %56, i32 %53
  %58 = add nsw i32 %57, %17
  %59 = shl i32 %58, 16
  %60 = ashr exact i32 %59, 16
  %61 = add nsw i32 %57, %13
  %62 = getelementptr inbounds [8 x i16], ptr @_witab.6, i64 0, i64 %30
  %63 = load i16, ptr %62, align 2, !tbaa !14
  %64 = sext i16 %63 to i32
  %65 = getelementptr inbounds [8 x i16], ptr @_fitab.7, i64 0, i64 %30
  %66 = load i16, ptr %65, align 2, !tbaa !14
  %67 = sext i16 %66 to i32
  %68 = shl i32 %61, 16
  %69 = ashr exact i32 %68, 16
  tail call fastcc void @update(i32 noundef 3, i32 noundef %23, i32 noundef %64, i32 noundef %67, i32 noundef %53, i32 noundef %60, i32 noundef %69, ptr noundef %2) #20
  br label %70

70:                                               ; preds = %52, %3
  %71 = phi i32 [ %27, %52 ], [ -1, %3 ]
  ret i32 %71
}

; Function Attrs: nofree nounwind optsize
declare noundef i64 @fread(ptr nocapture noundef, i64 noundef, i64 noundef, ptr nocapture noundef) local_unnamed_addr #4

; Function Attrs: nofree nounwind optsize uwtable
define internal fastcc i32 @pack_output(i32 noundef %0, i32 noundef %1) unnamed_addr #6 {
  %3 = load i32, ptr @pack_output.out_bits, align 4, !tbaa !25
  %4 = shl i32 %0, %3
  %5 = load i32, ptr @pack_output.out_buffer, align 4, !tbaa !25
  %6 = or i32 %5, %4
  store i32 %6, ptr @pack_output.out_buffer, align 4, !tbaa !25
  %7 = add nsw i32 %3, %1
  store i32 %7, ptr @pack_output.out_bits, align 4, !tbaa !25
  %8 = icmp sgt i32 %7, 7
  br i1 %8, label %9, label %17

9:                                                ; preds = %2
  %10 = add nsw i32 %7, -8
  store i32 %10, ptr @pack_output.out_bits, align 4, !tbaa !25
  %11 = lshr i32 %6, 8
  store i32 %11, ptr @pack_output.out_buffer, align 4, !tbaa !25
  %12 = load ptr, ptr @stdout, align 8, !tbaa !16
  %13 = shl i32 %6, 24
  %14 = ashr exact i32 %13, 24
  %15 = tail call i32 @fputc(i32 %14, ptr %12)
  %16 = load i32, ptr @pack_output.out_bits, align 4, !tbaa !25
  br label %17

17:                                               ; preds = %9, %2
  %18 = phi i32 [ %16, %9 ], [ %7, %2 ]
  %19 = icmp sgt i32 %18, 0
  %20 = zext i1 %19 to i32
  ret i32 %20
}

; Function Attrs: nofree nounwind optsize
declare noundef i32 @fclose(ptr nocapture noundef) local_unnamed_addr #4

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.end.p0(i64 immarg, ptr nocapture) #1

; Function Attrs: nofree nounwind
declare noundef i32 @fputc(i32 noundef, ptr nocapture noundef) local_unnamed_addr #7

; Function Attrs: mustprogress nofree norecurse nosync nounwind optsize willreturn memory(none) uwtable
define internal i32 @alaw2linear(i8 noundef zeroext %0) unnamed_addr #8 {
  %2 = xor i8 %0, 85
  %3 = zext i8 %2 to i32
  %4 = shl nuw nsw i32 %3, 4
  %5 = and i32 %4, 240
  %6 = lshr i32 %3, 4
  %7 = and i32 %6, 7
  switch i32 %7, label %12 [
    i32 0, label %8
    i32 1, label %10
  ]

8:                                                ; preds = %1
  %9 = or i32 %5, 8
  br label %16

10:                                               ; preds = %1
  %11 = or i32 %5, 264
  br label %16

12:                                               ; preds = %1
  %13 = or i32 %5, 264
  %14 = add nsw i32 %7, -1
  %15 = shl nuw nsw i32 %13, %14
  br label %16

16:                                               ; preds = %12, %10, %8
  %17 = phi i32 [ %15, %12 ], [ %11, %10 ], [ %9, %8 ]
  %18 = sub nsw i32 0, %17
  %19 = icmp slt i8 %0, 0
  %20 = select i1 %19, i32 %17, i32 %18
  ret i32 %20
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind optsize willreturn memory(none) uwtable
define internal i32 @ulaw2linear(i8 noundef zeroext %0) unnamed_addr #8 {
  %2 = xor i8 %0, -1
  %3 = zext i8 %2 to i32
  %4 = shl nuw nsw i32 %3, 3
  %5 = and i32 %4, 120
  %6 = or i32 %5, 132
  %7 = lshr i32 %3, 4
  %8 = and i32 %7, 7
  %9 = shl nuw nsw i32 %6, %8
  %10 = icmp slt i8 %0, 0
  %11 = sub nsw i32 132, %9
  %12 = add nsw i32 %9, -132
  %13 = select i1 %10, i32 %12, i32 %11
  ret i32 %13
}

; Function Attrs: nofree norecurse nosync nounwind optsize memory(argmem: read) uwtable
define internal fastcc i32 @predictor_zero(ptr nocapture noundef readonly %0) unnamed_addr #9 {
  %2 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 6
  %3 = load i16, ptr %2, align 4, !tbaa !14
  %4 = ashr i16 %3, 2
  %5 = sext i16 %4 to i32
  %6 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 8
  %7 = load i16, ptr %6, align 4, !tbaa !14
  %8 = sext i16 %7 to i32
  %9 = tail call fastcc i32 @fmult(i32 noundef %5, i32 noundef %8) #19, !range !27
  br label %10

10:                                               ; preds = %10, %1
  %11 = phi i64 [ 1, %1 ], [ %22, %10 ]
  %12 = phi i32 [ %9, %1 ], [ %21, %10 ]
  %13 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 6, i64 %11
  %14 = load i16, ptr %13, align 2, !tbaa !14
  %15 = ashr i16 %14, 2
  %16 = sext i16 %15 to i32
  %17 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 8, i64 %11
  %18 = load i16, ptr %17, align 2, !tbaa !14
  %19 = sext i16 %18 to i32
  %20 = tail call fastcc i32 @fmult(i32 noundef %16, i32 noundef %19) #19, !range !27
  %21 = add nsw i32 %20, %12
  %22 = add nuw nsw i64 %11, 1
  %23 = icmp eq i64 %22, 6
  br i1 %23, label %24, label %10, !llvm.loop !28

24:                                               ; preds = %10
  %.lcssa = phi i32 [ %21, %10 ]
  ret i32 %.lcssa
}

; Function Attrs: nofree norecurse nosync nounwind optsize memory(argmem: read) uwtable
define internal fastcc i32 @predictor_pole(ptr nocapture noundef readonly %0) unnamed_addr #9 {
  %2 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 5
  %3 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 5, i64 1
  %4 = load i16, ptr %3, align 2, !tbaa !14
  %5 = ashr i16 %4, 2
  %6 = sext i16 %5 to i32
  %7 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 9
  %8 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 9, i64 1
  %9 = load i16, ptr %8, align 2, !tbaa !14
  %10 = sext i16 %9 to i32
  %11 = tail call fastcc i32 @fmult(i32 noundef %6, i32 noundef %10) #19, !range !27
  %12 = load i16, ptr %2, align 8, !tbaa !14
  %13 = ashr i16 %12, 2
  %14 = sext i16 %13 to i32
  %15 = load i16, ptr %7, align 8, !tbaa !14
  %16 = sext i16 %15 to i32
  %17 = tail call fastcc i32 @fmult(i32 noundef %14, i32 noundef %16) #19, !range !27
  %18 = add nsw i32 %17, %11
  ret i32 %18
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind optsize willreturn memory(argmem: read) uwtable
define internal fastcc i32 @step_size(ptr nocapture noundef readonly %0) unnamed_addr #10 {
  %2 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 4
  %3 = load i16, ptr %2, align 2, !tbaa !29
  %4 = icmp sgt i16 %3, 255
  br i1 %4, label %5, label %9

5:                                                ; preds = %1
  %6 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 1
  %7 = load i16, ptr %6, align 8, !tbaa !13
  %8 = sext i16 %7 to i32
  br label %31

9:                                                ; preds = %1
  %10 = load i64, ptr %0, align 8, !tbaa !7
  %11 = lshr i64 %10, 6
  %12 = trunc i64 %11 to i32
  %13 = getelementptr inbounds %struct.g72x_state, ptr %0, i64 0, i32 1
  %14 = load i16, ptr %13, align 8, !tbaa !13
  %15 = sext i16 %14 to i32
  %16 = sub nsw i32 %15, %12
  %17 = ashr i16 %3, 2
  %18 = sext i16 %17 to i32
  %19 = icmp sgt i32 %16, 0
  br i1 %19, label %20, label %24

20:                                               ; preds = %9
  %21 = mul nsw i32 %16, %18
  %22 = ashr i32 %21, 6
  %23 = add nsw i32 %22, %12
  br label %31

24:                                               ; preds = %9
  %25 = icmp slt i32 %16, 0
  br i1 %25, label %26, label %31

26:                                               ; preds = %24
  %27 = mul nsw i32 %16, %18
  %28 = add nsw i32 %27, 63
  %29 = ashr i32 %28, 6
  %30 = add nsw i32 %29, %12
  br label %31

31:                                               ; preds = %26, %24, %20, %5
  %32 = phi i32 [ %8, %5 ], [ %23, %20 ], [ %30, %26 ], [ %12, %24 ]
  ret i32 %32
}

; Function Attrs: nofree nosync nounwind optsize memory(read, inaccessiblemem: none) uwtable
define internal fastcc i32 @quantize(i32 noundef %0, i32 noundef %1, ptr nocapture noundef readonly %2, i32 noundef %3) unnamed_addr #11 {
  %5 = tail call i32 @llvm.abs.i32(i32 %0, i1 true)
  %6 = shl i32 %5, 16
  %7 = ashr i32 %6, 17
  br label %8

8:                                                ; preds = %14, %4
  %9 = phi i32 [ %16, %14 ], [ 0, %4 ]
  %10 = phi ptr [ %15, %14 ], [ @power2, %4 ]
  %11 = load i16, ptr %10, align 2, !tbaa !14
  %12 = sext i16 %11 to i32
  %13 = icmp slt i32 %7, %12
  br i1 %13, label %18, label %14

14:                                               ; preds = %8
  %15 = getelementptr inbounds i16, ptr %10, i64 1
  %16 = add nuw nsw i32 %9, 1
  %17 = icmp eq i32 %16, 15
  br i1 %17, label %18, label %8, !llvm.loop !30

18:                                               ; preds = %14, %8
  %19 = phi i32 [ %9, %8 ], [ 15, %14 ]
  %20 = ashr exact i32 %6, 9
  %21 = shl i32 %19, 16
  %22 = ashr exact i32 %21, 16
  %23 = ashr i32 %20, %22
  %24 = and i32 %23, 127
  %25 = lshr exact i32 %21, 9
  %26 = lshr i32 %1, 2
  %27 = sub nsw i32 %25, %26
  %28 = add nsw i32 %27, %24
  %29 = shl i32 %28, 16
  %30 = ashr exact i32 %29, 16
  %31 = icmp sgt i32 %3, 0
  br i1 %31, label %32, label %45

32:                                               ; preds = %38, %18
  %33 = phi i32 [ %40, %38 ], [ 0, %18 ]
  %34 = phi ptr [ %39, %38 ], [ %2, %18 ]
  %35 = load i16, ptr %34, align 2, !tbaa !14
  %36 = sext i16 %35 to i32
  %37 = icmp slt i32 %30, %36
  br i1 %37, label %42, label %38

38:                                               ; preds = %32
  %39 = getelementptr inbounds i16, ptr %34, i64 1
  %40 = add nuw nsw i32 %33, 1
  %41 = icmp eq i32 %40, %3
  br i1 %41, label %42, label %32, !llvm.loop !30

42:                                               ; preds = %38, %32
  %43 = phi i32 [ %33, %32 ], [ %3, %38 ]
  %44 = icmp slt i32 %0, 0
  br i1 %44, label %47, label %52

45:                                               ; preds = %18
  %46 = icmp slt i32 %0, 0
  br i1 %46, label %47, label %54

47:                                               ; preds = %45, %42
  %48 = phi i32 [ 0, %45 ], [ %43, %42 ]
  %49 = shl i32 %3, 1
  %50 = or i32 %49, 1
  %51 = sub i32 %50, %48
  br label %57

52:                                               ; preds = %42
  %53 = icmp eq i32 %43, 0
  br i1 %53, label %54, label %57

54:                                               ; preds = %52, %45
  %55 = shl i32 %3, 1
  %56 = or i32 %55, 1
  br label %57

57:                                               ; preds = %54, %52, %47
  %58 = phi i32 [ %51, %47 ], [ %56, %54 ], [ %43, %52 ]
  ret i32 %58
}

; Function Attrs: nofree nosync nounwind optsize memory(argmem: readwrite) uwtable
define internal fastcc void @update(i32 noundef %0, i32 noundef %1, i32 noundef %2, i32 noundef %3, i32 noundef %4, i32 noundef %5, i32 noundef %6, ptr nocapture noundef %7) unnamed_addr #12 {
  %9 = lshr i32 %6, 31
  %10 = trunc i32 %9 to i16
  %11 = and i32 %4, 32767
  %12 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 10
  %13 = load i8, ptr %12, align 4, !tbaa !15
  %14 = icmp eq i8 %13, 0
  br i1 %14, label %32, label %15

15:                                               ; preds = %8
  %16 = load i64, ptr %7, align 8, !tbaa !7
  %17 = trunc i64 %16 to i32
  %18 = shl i32 %17, 1
  %19 = icmp sgt i32 %18, 655359
  %20 = lshr i32 %17, 10
  %21 = and i32 %20, 31
  %22 = or i32 %21, 32
  %23 = ashr i32 %18, 16
  %24 = shl i32 %22, %23
  %25 = shl i32 %24, 16
  %26 = ashr exact i32 %25, 16
  %27 = select i1 %19, i32 31744, i32 %26
  %28 = ashr i32 %27, 1
  %29 = add nsw i32 %28, %27
  %30 = ashr i32 %29, 1
  %31 = icmp sgt i32 %11, %30
  br label %32

32:                                               ; preds = %15, %8
  %33 = phi i1 [ false, %8 ], [ %31, %15 ]
  %34 = sub nsw i32 %2, %1
  %35 = lshr i32 %34, 5
  %36 = add i32 %35, %1
  %37 = trunc i32 %36 to i16
  %38 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 1
  store i16 %37, ptr %38, align 8, !tbaa !13
  %39 = shl i32 %36, 16
  %40 = icmp slt i32 %39, 35651584
  br i1 %40, label %43, label %41

41:                                               ; preds = %32
  %42 = icmp ugt i32 %39, 335544320
  br i1 %42, label %43, label %45

43:                                               ; preds = %41, %32
  %44 = phi i16 [ 544, %32 ], [ 5120, %41 ]
  store i16 %44, ptr %38, align 8, !tbaa !13
  br label %45

45:                                               ; preds = %43, %41
  %46 = phi i16 [ %37, %41 ], [ %44, %43 ]
  %47 = sext i16 %46 to i64
  %48 = load i64, ptr %7, align 8, !tbaa !7
  %49 = sub nsw i64 0, %48
  %50 = ashr i64 %49, 6
  %51 = add i64 %48, %47
  %52 = add i64 %51, %50
  store i64 %52, ptr %7, align 8, !tbaa !7
  %53 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 5
  %54 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 5, i64 1
  br i1 %33, label %55, label %56

55:                                               ; preds = %45
  tail call void @llvm.memset.p0.i64(ptr noundef nonnull align 8 dereferenceable(16) %53, i8 0, i64 16, i1 false)
  br label %143

56:                                               ; preds = %45
  %57 = load i16, ptr %54, align 2, !tbaa !14
  %58 = ashr i16 %57, 7
  %59 = sub i16 %57, %58
  %60 = icmp eq i32 %6, 0
  br i1 %60, label %96, label %61

61:                                               ; preds = %56
  %62 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 7
  %63 = load i16, ptr %62, align 8, !tbaa !14
  %64 = icmp eq i16 %63, %10
  %65 = load i16, ptr %53, align 8, !tbaa !14
  %66 = sub i16 0, %65
  %67 = select i1 %64, i16 %66, i16 %65
  %68 = icmp slt i16 %67, -8191
  br i1 %68, label %69, label %71

69:                                               ; preds = %61
  %70 = add i16 %59, -256
  br label %78

71:                                               ; preds = %61
  %72 = icmp sgt i16 %67, 8191
  br i1 %72, label %73, label %75

73:                                               ; preds = %71
  %74 = add i16 %59, 255
  br label %78

75:                                               ; preds = %71
  %76 = ashr i16 %67, 5
  %77 = add i16 %76, %59
  br label %78

78:                                               ; preds = %75, %73, %69
  %79 = phi i16 [ %70, %69 ], [ %74, %73 ], [ %77, %75 ]
  %80 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 7, i64 1
  %81 = load i16, ptr %80, align 2, !tbaa !14
  %82 = sext i16 %81 to i32
  %83 = icmp eq i32 %9, %82
  br i1 %83, label %90, label %84

84:                                               ; preds = %78
  %85 = icmp slt i16 %79, -12159
  br i1 %85, label %100, label %86

86:                                               ; preds = %84
  %87 = icmp sgt i16 %79, 12415
  %88 = add nsw i16 %79, -128
  %89 = select i1 %87, i16 12288, i16 %88
  br label %100

90:                                               ; preds = %78
  %91 = icmp slt i16 %79, -12415
  br i1 %91, label %100, label %92

92:                                               ; preds = %90
  %93 = icmp sgt i16 %79, 12159
  %94 = add i16 %79, 128
  %95 = select i1 %93, i16 12288, i16 %94
  br label %100

96:                                               ; preds = %56
  store i16 %59, ptr %54, align 2, !tbaa !14
  %97 = load i16, ptr %53, align 8, !tbaa !14
  %98 = ashr i16 %97, 8
  %99 = sub i16 %97, %98
  br label %108

100:                                              ; preds = %92, %90, %86, %84
  %101 = phi i16 [ %95, %92 ], [ %89, %86 ], [ -12288, %90 ], [ -12288, %84 ]
  store i16 %101, ptr %54, align 2, !tbaa !14
  %102 = ashr i16 %65, 8
  %103 = sub i16 %65, %102
  br i1 %64, label %104, label %106

104:                                              ; preds = %100
  %105 = add i16 %103, 192
  br label %108

106:                                              ; preds = %100
  %107 = add i16 %103, -192
  br label %108

108:                                              ; preds = %106, %104, %96
  %109 = phi i16 [ %99, %96 ], [ %105, %104 ], [ %107, %106 ]
  %110 = phi i16 [ %59, %96 ], [ %101, %104 ], [ %101, %106 ]
  store i16 %109, ptr %53, align 8, !tbaa !14
  %111 = sub i16 15360, %110
  %112 = sext i16 %109 to i32
  %113 = sext i16 %111 to i32
  %114 = sub nsw i32 0, %113
  %115 = icmp slt i32 %112, %114
  br i1 %115, label %116, label %118

116:                                              ; preds = %108
  %117 = trunc i32 %114 to i16
  br label %120

118:                                              ; preds = %108
  %119 = icmp sgt i16 %109, %111
  br i1 %119, label %120, label %122

120:                                              ; preds = %118, %116
  %121 = phi i16 [ %117, %116 ], [ %111, %118 ]
  store i16 %121, ptr %53, align 8, !tbaa !14
  br label %122

122:                                              ; preds = %120, %118
  %123 = icmp eq i32 %0, 5
  %124 = icmp eq i32 %11, 0
  %125 = select i1 %123, i16 9, i16 8
  br label %126

126:                                              ; preds = %140, %122
  %127 = phi i64 [ 0, %122 ], [ %141, %140 ]
  %128 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 6, i64 %127
  %129 = load i16, ptr %128, align 2, !tbaa !14
  %130 = ashr i16 %129, %125
  %131 = sub i16 %129, %130
  store i16 %131, ptr %128, align 2, !tbaa !14
  br i1 %124, label %140, label %132

132:                                              ; preds = %126
  %133 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 8, i64 %127
  %134 = load i16, ptr %133, align 2, !tbaa !14
  %135 = sext i16 %134 to i32
  %136 = xor i32 %135, %4
  %137 = icmp sgt i32 %136, -1
  %138 = select i1 %137, i16 128, i16 -128
  %139 = add i16 %138, %131
  store i16 %139, ptr %128, align 2, !tbaa !14
  br label %140

140:                                              ; preds = %132, %126
  %141 = add nuw nsw i64 %127, 1
  %142 = icmp eq i64 %141, 6
  br i1 %142, label %143, label %126, !llvm.loop !31

143:                                              ; preds = %140, %55
  %144 = phi i16 [ undef, %55 ], [ %110, %140 ]
  %145 = getelementptr i8, ptr %7, i64 38
  %146 = getelementptr i8, ptr %7, i64 36
  tail call void @llvm.memmove.p0.p0.i64(ptr noundef nonnull align 2 dereferenceable(10) %145, ptr noundef nonnull align 2 dereferenceable(10) %146, i64 10, i1 false), !tbaa !14
  %147 = icmp eq i32 %11, 0
  br i1 %147, label %148, label %151

148:                                              ; preds = %143
  %149 = icmp sgt i32 %4, -1
  %150 = select i1 %149, i16 32, i16 -992
  br label %173

151:                                              ; preds = %157, %143
  %152 = phi i32 [ %159, %157 ], [ 0, %143 ]
  %153 = phi ptr [ %158, %157 ], [ @power2, %143 ]
  %154 = load i16, ptr %153, align 2, !tbaa !14
  %155 = sext i16 %154 to i32
  %156 = icmp slt i32 %11, %155
  br i1 %156, label %161, label %157

157:                                              ; preds = %151
  %158 = getelementptr inbounds i16, ptr %153, i64 1
  %159 = add nuw nsw i32 %152, 1
  %160 = icmp eq i32 %159, 15
  br i1 %160, label %161, label %151, !llvm.loop !30

161:                                              ; preds = %157, %151
  %162 = phi i32 [ %152, %151 ], [ 15, %157 ]
  %163 = shl i32 %162, 16
  %164 = ashr exact i32 %163, 16
  %165 = ashr exact i32 %163, 10
  %166 = shl nuw nsw i32 %11, 6
  %167 = lshr i32 %166, %164
  %168 = add nsw i32 %165, 64512
  %169 = icmp slt i32 %4, 0
  %170 = select i1 %169, i32 %168, i32 %165
  %171 = add nsw i32 %170, %167
  %172 = trunc i32 %171 to i16
  br label %173

173:                                              ; preds = %161, %148
  %174 = phi i16 [ %172, %161 ], [ %150, %148 ]
  store i16 %174, ptr %146, align 4, !tbaa !14
  %175 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 9
  %176 = load i16, ptr %175, align 8, !tbaa !14
  %177 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 9, i64 1
  store i16 %176, ptr %177, align 2, !tbaa !14
  %178 = icmp eq i32 %5, 0
  br i1 %178, label %224, label %179

179:                                              ; preds = %173
  %180 = icmp sgt i32 %5, 0
  br i1 %180, label %181, label %200

181:                                              ; preds = %187, %179
  %182 = phi i32 [ %189, %187 ], [ 0, %179 ]
  %183 = phi ptr [ %188, %187 ], [ @power2, %179 ]
  %184 = load i16, ptr %183, align 2, !tbaa !14
  %185 = sext i16 %184 to i32
  %186 = icmp sgt i32 %185, %5
  br i1 %186, label %191, label %187

187:                                              ; preds = %181
  %188 = getelementptr inbounds i16, ptr %183, i64 1
  %189 = add nuw nsw i32 %182, 1
  %190 = icmp eq i32 %189, 15
  br i1 %190, label %191, label %181, !llvm.loop !30

191:                                              ; preds = %187, %181
  %192 = phi i32 [ %182, %181 ], [ 15, %187 ]
  %193 = shl i32 %192, 16
  %194 = ashr exact i32 %193, 16
  %195 = lshr exact i32 %193, 10
  %196 = shl i32 %5, 6
  %197 = ashr i32 %196, %194
  %198 = add i32 %197, %195
  %199 = trunc i32 %198 to i16
  br label %224

200:                                              ; preds = %179
  %201 = icmp ugt i32 %5, -32768
  br i1 %201, label %202, label %224

202:                                              ; preds = %200
  %203 = sub nsw i32 0, %5
  br label %204

204:                                              ; preds = %210, %202
  %205 = phi i32 [ %212, %210 ], [ 0, %202 ]
  %206 = phi ptr [ %211, %210 ], [ @power2, %202 ]
  %207 = load i16, ptr %206, align 2, !tbaa !14
  %208 = sext i16 %207 to i32
  %209 = icmp sgt i32 %208, %203
  br i1 %209, label %214, label %210

210:                                              ; preds = %204
  %211 = getelementptr inbounds i16, ptr %206, i64 1
  %212 = add nuw nsw i32 %205, 1
  %213 = icmp eq i32 %212, 15
  br i1 %213, label %214, label %204, !llvm.loop !30

214:                                              ; preds = %210, %204
  %215 = phi i32 [ %205, %204 ], [ 15, %210 ]
  %216 = shl i32 %215, 16
  %217 = ashr exact i32 %216, 16
  %218 = lshr exact i32 %216, 10
  %219 = shl nuw nsw i32 %203, 6
  %220 = lshr i32 %219, %217
  %221 = add nuw nsw i32 %220, %218
  %222 = trunc i32 %221 to i16
  %223 = add i16 %222, -1024
  br label %224

224:                                              ; preds = %214, %200, %191, %173
  %225 = phi i16 [ %199, %191 ], [ %223, %214 ], [ 32, %173 ], [ -992, %200 ]
  store i16 %225, ptr %175, align 8, !tbaa !14
  %226 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 7
  %227 = load i16, ptr %226, align 8, !tbaa !14
  %228 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 7, i64 1
  store i16 %227, ptr %228, align 2, !tbaa !14
  store i16 %10, ptr %226, align 8, !tbaa !14
  %229 = icmp slt i16 %144, -11776
  %230 = xor i1 %33, true
  %231 = and i1 %229, %230
  %232 = zext i1 %231 to i8
  store i8 %232, ptr %12, align 4, !tbaa !15
  %233 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 2
  %234 = load i16, ptr %233, align 2, !tbaa !32
  %235 = sext i16 %234 to i32
  %236 = sub nsw i32 %3, %235
  %237 = lshr i32 %236, 5
  %238 = trunc i32 %237 to i16
  %239 = add i16 %234, %238
  store i16 %239, ptr %233, align 2, !tbaa !32
  %240 = shl i32 %3, 2
  %241 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 3
  %242 = load i16, ptr %241, align 4, !tbaa !33
  %243 = sext i16 %242 to i32
  %244 = sub nsw i32 %240, %243
  %245 = lshr i32 %244, 7
  %246 = trunc i32 %245 to i16
  %247 = add i16 %242, %246
  store i16 %247, ptr %241, align 4, !tbaa !33
  br i1 %33, label %248, label %250

248:                                              ; preds = %224
  %249 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 4
  store i16 256, ptr %249, align 2, !tbaa !29
  br label %290

250:                                              ; preds = %224
  %251 = icmp slt i32 %1, 1536
  br i1 %251, label %252, label %260

252:                                              ; preds = %250
  %253 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 4
  %254 = load i16, ptr %253, align 2, !tbaa !29
  %255 = sext i16 %254 to i32
  %256 = sub nsw i32 512, %255
  %257 = ashr i32 %256, 4
  %258 = trunc i32 %257 to i16
  %259 = add i16 %254, %258
  store i16 %259, ptr %253, align 2, !tbaa !29
  br label %290

260:                                              ; preds = %250
  br i1 %231, label %261, label %269

261:                                              ; preds = %260
  %262 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 4
  %263 = load i16, ptr %262, align 2, !tbaa !29
  %264 = sext i16 %263 to i32
  %265 = sub nsw i32 512, %264
  %266 = ashr i32 %265, 4
  %267 = trunc i32 %266 to i16
  %268 = add i16 %263, %267
  store i16 %268, ptr %262, align 2, !tbaa !29
  br label %290

269:                                              ; preds = %260
  %270 = sext i16 %239 to i32
  %271 = shl nsw i32 %270, 2
  %272 = sext i16 %247 to i32
  %273 = sub nsw i32 %271, %272
  %274 = tail call i32 @llvm.abs.i32(i32 %273, i1 true)
  %275 = ashr i32 %272, 3
  %276 = icmp slt i32 %274, %275
  %277 = getelementptr inbounds %struct.g72x_state, ptr %7, i64 0, i32 4
  %278 = load i16, ptr %277, align 2, !tbaa !29
  %279 = sext i16 %278 to i32
  br i1 %276, label %285, label %280

280:                                              ; preds = %269
  %281 = sub nsw i32 512, %279
  %282 = ashr i32 %281, 4
  %283 = trunc i32 %282 to i16
  %284 = add i16 %278, %283
  store i16 %284, ptr %277, align 2, !tbaa !29
  br label %290

285:                                              ; preds = %269
  %286 = sub nsw i32 0, %279
  %287 = ashr i32 %286, 4
  %288 = trunc i32 %287 to i16
  %289 = add i16 %278, %288
  store i16 %289, ptr %277, align 2, !tbaa !29
  br label %290

290:                                              ; preds = %285, %280, %261, %252, %248
  ret void
}

; Function Attrs: nocallback nofree nounwind willreturn memory(argmem: readwrite)
declare void @llvm.memmove.p0.p0.i64(ptr nocapture writeonly, ptr nocapture readonly, i64, i1 immarg) #13

; Function Attrs: nocallback nofree nosync nounwind speculatable willreturn memory(none)
declare i32 @llvm.abs.i32(i32, i1 immarg) #14

; Function Attrs: nofree norecurse nosync nounwind optsize memory(none) uwtable
define internal fastcc i32 @fmult(i32 noundef %0, i32 noundef %1) unnamed_addr #15 {
  %3 = icmp sgt i32 %0, 0
  %4 = sub nsw i32 0, %0
  %5 = and i32 %4, 8191
  %6 = select i1 %3, i32 %0, i32 %5
  br label %7

7:                                                ; preds = %13, %2
  %8 = phi i32 [ %15, %13 ], [ 0, %2 ]
  %9 = phi ptr [ %14, %13 ], [ @power2, %2 ]
  %10 = load i16, ptr %9, align 2, !tbaa !14
  %11 = sext i16 %10 to i32
  %12 = icmp slt i32 %6, %11
  br i1 %12, label %17, label %13

13:                                               ; preds = %7
  %14 = getelementptr inbounds i16, ptr %9, i64 1
  %15 = add nuw nsw i32 %8, 1
  %16 = icmp eq i32 %15, 15
  br i1 %16, label %17, label %7, !llvm.loop !30

17:                                               ; preds = %13, %7
  %18 = phi i32 [ %8, %7 ], [ 15, %13 ]
  %19 = trunc i32 %18 to i16
  %20 = add i16 %19, -6
  %21 = icmp eq i32 %6, 0
  br i1 %21, label %30, label %22

22:                                               ; preds = %17
  %23 = sext i16 %20 to i32
  %24 = icmp sgt i16 %20, -1
  br i1 %24, label %25, label %27

25:                                               ; preds = %22
  %26 = ashr i32 %6, %23
  br label %30

27:                                               ; preds = %22
  %28 = sub nsw i32 0, %23
  %29 = shl i32 %6, %28
  br label %30

30:                                               ; preds = %27, %25, %17
  %31 = phi i32 [ 32, %17 ], [ %26, %25 ], [ %29, %27 ]
  %32 = trunc i32 %1 to i16
  %33 = lshr i16 %32, 6
  %34 = and i16 %33, 15
  %35 = add nsw i16 %34, -13
  %36 = add i16 %35, %20
  %37 = shl i32 %31, 16
  %38 = ashr exact i32 %37, 16
  %39 = and i32 %1, 63
  %40 = mul nsw i32 %38, %39
  %41 = add nsw i32 %40, 48
  %42 = lshr i32 %41, 4
  %43 = sext i16 %36 to i32
  %44 = icmp sgt i16 %36, -1
  br i1 %44, label %45, label %48

45:                                               ; preds = %30
  %46 = shl i32 %42, %43
  %47 = and i32 %46, 32767
  br label %53

48:                                               ; preds = %30
  %49 = shl i32 %42, 16
  %50 = ashr exact i32 %49, 16
  %51 = sub nsw i32 0, %43
  %52 = ashr i32 %50, %51
  br label %53

53:                                               ; preds = %48, %45
  %54 = phi i32 [ %47, %45 ], [ %52, %48 ]
  %55 = xor i32 %1, %0
  %56 = icmp slt i32 %55, 0
  %57 = sub nsw i32 0, %54
  %58 = select i1 %56, i32 %57, i32 %54
  ret i32 %58
}

attributes #0 = { nounwind optsize uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #1 = { nocallback nofree nosync nounwind willreturn memory(argmem: readwrite) }
attributes #2 = { nocallback nofree nounwind willreturn memory(argmem: write) }
attributes #3 = { nounwind optsize memory(read, argmem: readwrite, inaccessiblemem: none) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #4 = { nofree nounwind optsize "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #5 = { noreturn nounwind optsize "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #6 = { nofree nounwind optsize uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #7 = { nofree nounwind }
attributes #8 = { mustprogress nofree norecurse nosync nounwind optsize willreturn memory(none) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #9 = { nofree norecurse nosync nounwind optsize memory(argmem: read) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #10 = { mustprogress nofree norecurse nosync nounwind optsize willreturn memory(argmem: read) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #11 = { nofree nosync nounwind optsize memory(read, inaccessiblemem: none) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #12 = { nofree nosync nounwind optsize memory(argmem: readwrite) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #13 = { nocallback nofree nounwind willreturn memory(argmem: readwrite) }
attributes #14 = { nocallback nofree nosync nounwind speculatable willreturn memory(none) }
attributes #15 = { nofree norecurse nosync nounwind optsize memory(none) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #16 = { nounwind }
attributes #17 = { cold }
attributes #18 = { noreturn nounwind optsize }
attributes #19 = { optsize }
attributes #20 = { nounwind optsize }

!llvm.ident = !{!0, !0, !0, !0, !0, !0}
!llvm.module.flags = !{!1, !2, !3, !4, !5, !6}

!0 = !{!"clang version 17.0.6 (https://github.com/Casperento/llvm-project.git 9b0073551ece0d22bf3378af2b03e456a26031b6)"}
!1 = !{i32 1, !"wchar_size", i32 4}
!2 = !{i32 8, !"PIC Level", i32 2}
!3 = !{i32 7, !"PIE Level", i32 2}
!4 = !{i32 7, !"uwtable", i32 2}
!5 = !{i32 1, !"ThinLTO", i32 0}
!6 = !{i32 1, !"EnableSplitLTOUnit", i32 1}
!7 = !{!8, !9, i64 0}
!8 = !{!"g72x_state", !9, i64 0, !12, i64 8, !12, i64 10, !12, i64 12, !12, i64 14, !10, i64 16, !10, i64 20, !10, i64 32, !10, i64 36, !10, i64 48, !10, i64 52}
!9 = !{!"long", !10, i64 0}
!10 = !{!"omnipotent char", !11, i64 0}
!11 = !{!"Simple C/C++ TBAA"}
!12 = !{!"short", !10, i64 0}
!13 = !{!8, !12, i64 8}
!14 = !{!12, !12, i64 0}
!15 = !{!8, !10, i64 52}
!16 = !{!17, !17, i64 0}
!17 = !{!"any pointer", !10, i64 0}
!18 = !{!10, !10, i64 0}
!19 = distinct !{!19, !20}
!20 = !{!"llvm.loop.mustprogress"}
!21 = !{ptr @g721_encoder, ptr @g723_24_encoder, ptr @g723_40_encoder}
!22 = !{i32 0, i32 2}
!23 = distinct !{!23, !20}
!24 = distinct !{!24, !20}
!25 = !{!26, !26, i64 0}
!26 = !{!"int", !10, i64 0}
!27 = !{i32 -32767, i32 32768}
!28 = distinct !{!28, !20}
!29 = !{!8, !12, i64 14}
!30 = distinct !{!30, !20}
!31 = distinct !{!31, !20}
!32 = !{!8, !12, i64 10}
!33 = !{!8, !12, i64 12}
